    ...
    Partial Public Class TcpSever
        ...
        Public Sub Start() Implements ICommSever(Of TcpClientMember).Start
            Try
                If Not Me.CommSeverState = ICommSever(Of TcpClientMember).StateModel.running Then
                    ...
                    If eListen() = SocketError.Success Then
                        ...
                        eAccept()
                        '记录
                        CommSeverLog.LogSuccess(Log.CommSeverLogElement(Of String, TcpSever).MsgProcessDescriptionModel.start)
                    Else
                        '记录
                        CommSeverLog.LogError(Log.CommSeverLogElement(Of String, TcpSever).MsgProcessDescriptionModel.stop)
                    End If
                End If
            Catch ex As Exception
                ...
                '记录当前状态
                CommSeverLog.LogError(Log.CommSeverLogElement(Of String, TcpSever).MsgProcessDescriptionModel.start, ex.ToString)
            End Try
        End Sub
        ...
        Private Function eListen() As SocketError
            ...
            Catch Skex As SocketException
                ...
                '记录
                CommSeverLog.LogError(Log.CommSeverLogElement(Of String, TcpSever).MsgProcessDescriptionModel.start, "SkErr:" & Skex.ToString)
                Return Skex.SocketErrorCode
            Catch ex As Exception
                '记录
                CommSeverLog.LogError(Log.CommSeverLogElement(Of String, TcpSever).MsgProcessDescriptionModel.start, "SkErr:" & ex.ToString)
            End Try
                ...
                CommSeverSocket.BeginAccept(New AsyncCallback(Sub(Iar As IAsyncResult)
                                                                  Try
                                                                      ...
                                                                      Catch Skex As SocketException
                                                                          ...
                                                                          Me.CommSeverSocketState = Skex.SocketErrorCode
                                                                          '记录
                                                                          CommSeverLog.LogError(Log.CommSeverLogElement(Of String, TcpSever).MsgProcessDescriptionModel.recvconn, "SkErr:" & Skex.ToString)
                                                                      Catch ex As Exception
                                                                          '记录
                                                                          CommSeverLog.LogError(Log.CommSeverLogElement(Of String, TcpSever).MsgProcessDescriptionModel.recvconn, ex.ToString)
                                                                      End Try
                                                                  Catch ex As Exception
                                                                      Exit Sub
                                                                  Finally
        ...
        Private Sub RegMemberAction(ByRef mem As TcpClientMember) Handles Me.EventRegMember
            ...
            '记录
            CommSeverLog.LogSuccess(Log.CommSeverLogElement(Of String, TcpSever).MsgProcessDescriptionModel.recvconn, mem.CommCofigStr)
        End Sub
    End Class
End Namespace
