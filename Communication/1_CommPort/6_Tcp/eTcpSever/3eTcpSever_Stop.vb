        ...
        Public Sub [Stop]() Implements ICommSever(Of TcpClientMember).Stop
            If Not Me.CommSeverState = ICommSever(Of TcpClientMember).StateModel.stopped Then
                ...
                Try
                    ...
                    '记录
                    CommSeverLog.LogSuccess(Log.CommSeverLogElement(Of String, eTcpSever).MsgProcessDescriptionModel.stop)
                Catch exSk As SocketException
                    '更新状态
                    Me.CommSeverState = ICommSever(Of TcpClientMember).StateModel.error
                    '记录
                    CommSeverLog.LogError(Log.CommSeverLogElement(Of String, eTcpSever).MsgProcessDescriptionModel.stop, "SkErr:" & exSk.ErrorCode)
                Catch ex As Exception
                    '更新状态
                    Me.CommSeverState = ICommSever(Of TcpClientMember).StateModel.error
                    '记录
                    CommSeverLog.LogError(Log.CommSeverLogElement(Of String, eTcpSever).MsgProcessDescriptionModel.stop, ex.ToString)
                End Try
            End If
        End Sub
