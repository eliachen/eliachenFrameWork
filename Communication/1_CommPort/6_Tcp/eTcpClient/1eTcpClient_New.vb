...
Namespace CommPort
    Partial Public Class eTcpClient
        ...
        Public Property CommLog As Log.CommPortLogManager(Of eTcpClientLog) = _
                                                                New Log.CommPortLogManager(Of eTcpClientLog)(Me)
        ...
        Sub New(ByVal ConfigStr As String)
            ...
            If String.IsNullOrEmpty(Me.CommCofigStr) Then
                CommLog.LogError(Log.CommPortLogElement(Of eTcpClientLog).MsgProcessDescriptionModel.optionset,
                                 New eTcpClientLog(Me.ActivatedSocketState, "字符串配置"))
        ...
        Private Sub SkInitial()
            Try
                ...
                CommLog.LogSuccess(Log.CommPortLogElement(Of eTcpClientLog).MsgProcessDescriptionModel.optionset,
                                  New eTcpClientLog(Me.ActivatedSocketState, "保活设置"))
            Catch ex As Exception
                CommLog.LogError(Log.CommPortLogElement(Of eTcpClientLog).MsgProcessDescriptionModel.optionset,
                                 New eTcpClientLog(Me.ActivatedSocketState, "保活设置:" & ex.ToString))
            Catch exSk As SocketException
                CommLog.LogError(Log.CommPortLogElement(Of eTcpClientLog).MsgProcessDescriptionModel.optionset,
                                 New eTcpClientLog(exSk.SocketErrorCode, "保活设置"))
...
#End Region
        ...
        Public Sub ReConnect() Implements ICommPort.ReConnect
            If eDisConnect() = SocketError.Success Then
                '记录：
                CommLog.LogSuccess(Log.CommPortLogElement(Of eTcpClientLog).MsgProcessDescriptionModel.reconnect,
                                   New eTcpClientLog(Me.ActivatedSocketState, "断开连接"))
                Me.Connect()
            Else
                CommLog.LogError(Log.CommPortLogElement(Of eTcpClientLog).MsgProcessDescriptionModel.reconnect,
                                    New eTcpClientLog(Me.ActivatedSocketState, "断开连接"))
            End If
        End Sub
